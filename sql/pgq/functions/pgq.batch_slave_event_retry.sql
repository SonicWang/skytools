create or replace function pgq.batch_slave_event_retry(
    x_batch_id bigint,
    x_event_ids bigint[],
    x_retry_time timestamptz)
returns integer as $$
-- ----------------------------------------------------------------------
-- Function: pgq.batch_slave_event_retry(3a)
--
--     Put the events into retry queue, to be processed again later.
--
-- Parameters:
--      x_batch_id      - ID of active batch.
--      x_event_ids     - event ids in an array
...
returns integer as $$
-- ----------------------------------------------------------------------
-- Function: pgq.batch_slave_event_retry(3b)
--
--     Put the event ids into retry queue, to be processed later again.
--
-- Parameters:
--      x_batch_id      - ID of active batch.
--      x_event_ids     - event ids in an array
...
begin
    ...
    return pgq.batch_slave_event_retry(x_batch_id, x_event_ids, new_retry);
end;
$$ language plpgsql security definer;
